...
// ===== KNIGHT RIDER mapping LED1..LED8 =====
#define KR_N 8

// pos 0..7 v pořadí "od prvního do posledního"
static const uint8_t kr_bitpos[KR_N] = {
    (uint8_t)(16u+8u),  // 1
    (uint8_t)(16u+9u),  // 2
    (uint8_t)(16u+10u), // 3
    (uint8_t)(16u+11u), // 4
...
static uint32_t kr_mask3(int pos)
{
    uint32_t m = 0;
    ...
    // рядом с краем: 2 LED, но только когда JDEME K OKRAJI
    if (kr_dir > 0 && pos == (KR_N-2)) {
        // jdeme doprava -> svítí pos a poslední
        m |= 1u << kr_bitpos[pos];
        m |= 1u << kr_bitpos[pos + 1];
        return m;
    }
    if (kr_dir < 0 && pos == 1) {
        // jdeme doleva -> svítí první a pos
        m |= 1u << kr_bitpos[pos - 1];
        m |= 1u << kr_bitpos[pos];
        return m;
    }
...
//		HAL_Delay(50);
		...
		uint32_t step_ms = 20u + (raw_pot * (500u - 20u)) / 4095u;

		static uint32_t kr_next = 0;

		if ((int32_t)(now - kr_next) >= 0) {
		    kr_next = now + step_ms;

		    uint32_t mask = kr_mask3(kr_pos);
		    sct_value_ledmask((uint16_t)step_ms, 0, mask);

		    kr_pos += kr_dir;
		    if (kr_pos >= KR_N-1) { kr_pos = KR_N-1; kr_dir = -1; }
		    if (kr_pos <= 0)      { kr_pos = 0;      kr_dir = +1; }
		}
