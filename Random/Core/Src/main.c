...
// ===== DICE TRAINING =====
typedef enum { DICE_RUN, DICE_SLOW, DICE_STOP } dice_state_t;
static dice_state_t dice_state = DICE_RUN;

static uint8_t dice = 1;
static uint32_t step_ms = 30;
static uint32_t next_step = 0;
static uint32_t slow_timer = 0;

// jednoduchý LFSR random
static uint16_t lfsr = 0xACE1u;
static uint8_t rand_1_6(void)
{
...
//		HAL_Delay(50);
		...
		// tlačítko S1 – stop
		static uint8_t s1_prev = 1;
		uint8_t s1 = HAL_GPIO_ReadPin(S1_GPIO_Port, S1_Pin);
		if (s1_prev == 1 && s1 == 0) {
		    dice_state = DICE_STOP;
		}
		s1_prev = s1;

		// tlačítko S2 – restart
		static uint8_t s2_prev = 1;
		...
		if (dice_state != DICE_STOP) {
		    ...
		    if (dice_state == DICE_RUN) {
		        if (t0 == 0) t0 = now;
		        if (now - t0 > 1500) {
		            dice_state = DICE_SLOW;
		            slow_timer = now;
		        }
		    }
		    else if (dice_state == DICE_SLOW) {
		        if (now - slow_timer > 120) {
		            slow_timer = now;
		            if (step_ms < 250) step_ms += 20;
		            else dice_state = DICE_STOP;
		        }
		    }
		}
